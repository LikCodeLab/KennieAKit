...
import com.learkoo.utils.exception.FileCreateFailureException;
import com.learkoo.utils.exception.SdCardNotMountedException;
import com.learkoo.utils.exception.SdCardNotValidException;
import com.learkoo.utils.file.Storage.SdCard;
...
public class FStoreLoc {
	private static  String TAG = FStoreLoc.class.getSimpleName();
	...
	public void setBaseDirName(Context context, String baseDir) {
		...
		if (baseDirName != null) {
			if (!baseDirName.equals(baseDir)) lg.i(TAG, "不可重复设置不同的目录");
			return;
		}
		baseDirName = baseDir;
		saveCurrent(context);
	...
	private SdCard getAnother(SdCard current) {
		...
		lg.i(TAG , "getAnother----sdcardTo:" + sdcardTo);
		return sdcardTo;
	}
